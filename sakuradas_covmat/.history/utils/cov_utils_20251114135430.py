...
def network_covmat():
    ...
    for tr in stream_minute:
        ...
        if current_endtime < target_endtime:
            npts_to_add = int((target_endtime - current_endtime) * tr.stats.sampling_rate)
            if npts_to_add > 0:
    ...
    print(stream_minute)
    stream_minute.filter("bandpass", 1,4, corners=3, zerophase=True)
    stream_minute.plot()
...
if __name__ == "__main__":
    ...
    hdf5_endttime_jst = datetime.datetime(2022, 12, 8, 1, 0, 0) 
    network_covmat()
